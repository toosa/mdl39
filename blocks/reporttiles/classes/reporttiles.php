...
class block_reporttiles_reporttiles {
    ...
    public function reporttiles_icon($itemid, $blockinstanceid, $reportname) {
        ...
        // $file = $DB->get_record('files', array('itemid' => $itemid, 'filearea' => 'reporttiles'));
        $file = $DB->get_record_sql("SELECT * FROM {files} WHERE itemid = $itemid 
                                        AND component='block_reporttiles' AND filearea = 'reporttiles' AND filesize <> 0");
        if (empty($file)) {
