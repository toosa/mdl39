        ...
        switch($data->column){
            ...
            case 'lastaccess':
                $attempt = $DB->get_field_sql("SELECT MAX(sst.attempt) FROM {scorm_scoes_track} sst 
                    JOIN {scorm} s ON s.id = sst.scormid WHERE sst.userid = $row->id AND s.course = $courseid ");
                if (!empty($attempt)) {
