	...
	public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0){
		...
		switch ($data->column) {
            ...
            case 'wordscount':
                ...
                break;
            // case 'wordscount':
            //     $learnersql  = (new querylib)->get_learners('', $row->course);
            //     $messages = $DB->get_records_sql("SELECT fp.message
            //                     FROM {forum_posts} fp 
            //                     JOIN {forum_discussions} fd ON fp.discussion = fd.id 
            //                     WHERE 1 = 1 AND fp.userid IN ($learnersql) AND fd.forum = $row->id");
            //     foreach ($messages as $message) {
            //         $msgarray[] = strip_tags($message->message);
            //     }
            //     $msgs = implode(' ', $msgarray);
            //     $string = preg_replace('/\s+/', ' ', trim($msgs));
            //     if (!empty($string)) {
