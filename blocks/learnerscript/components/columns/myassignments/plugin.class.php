		...
		switch ($data->column) {
            ...
            case 'grademax':
                 if($reporttype == 'table'){
                    ...
                    $row->{$data->column} = isset($row->grademax) ? $row->grademax : 0 ;
                }
                break;
			case 'status':
				$courserecord = $DB->get_record('course', array('id' => $row->courseid));
				$completion_info = new completion_info($courserecord);
				$coursemodulecompletion = $DB->get_record_sql("SELECT id FROM {course_modules_completion} WHERE userid = $row->userid AND coursemoduleid = $row->activityid", array(), IGNORE_MULTIPLE);
