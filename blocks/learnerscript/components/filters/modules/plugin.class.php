...
class plugin_modules extends pluginbase {
    ...
    public function init() {
        ...
        $this->reporttypes = array('listofactivities','useractivities','student_performance','coursesoverview', 'popularresources', 'resources_accessed','gradedactivity');
    }
    ...
    public function filter_data($selectoption = true){
        ...
        if($this->report->type == 'gradedactivity'){
            ...
            $modules = $DB->get_records_sql("SELECT id, name FROM {modules} WHERE visible = 1 AND id IN ($resourceslist)");
        } else{
            $modules = $DB->get_records_sql('SELECT id, name FROM {modules} WHERE visible = 1');
        }
        if(!empty($modules)) {
