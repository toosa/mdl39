...
class report_myquizs extends reportbase implements report {
    ...
    function select() {
        $this->sql = "SELECT DISTINCT q.id, q.name as name, m.name as modname, cm.id AS activityid , ra.userid AS userid, ROUND(q.grade, 2) AS grademax, q.course AS courseid, q.* ";
    ...
    function search() {
        if (isset($this->search) && $this->search) {
            $fields = array('q.name','c.fullname','q.grade','m.name');
    ...
    public function column_queries($columnname, $quizid) {
        ...
        switch ($columnname) {
            ...
            case 'gradepass':
                $identy = 'gi.iteminstance';
                $query = "SELECT ROUND(gi.gradepass) AS gradepass FROM {grade_items} as gi WHERE gi.itemmodule = 'quiz' $where ";
                break;
            case 'finalgrade':
                $identy = 'gi.iteminstance';
            ...
            case 'grademax':
                $identy = 'q1.id';
                $query = "SELECT ROUND(q1.grade) AS grademax FROM {quiz} q1
                           WHERE 1 = 1 $where  ";
            break;
            case 'highestgrade':
                $identy = 'gi.iteminstance';
