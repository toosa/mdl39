                                 ...
                                 WHERE lsl.crud = 'r' AND lsl.contextlevel = 70  AND lsl.anonymous = 0 AND u.id IN ($learnersql)
                                  ...
                                  AS c1,
                                   (SELECT COUNT('X') as numviews 
                                     ...
                                     JOIN {scorm} q ON q.id = cm.instance 
                                     JOIN {modules} m ON m.id = cm.module
                                     WHERE  lsl.crud = 'r' AND lsl.contextlevel = 70 AND lsl.userid > 2 AND u.id IN ($learnersql) AND lsl.anonymous = 0 AND u.confirmed = 1 AND u.deleted = 0 AND m.name = 'scorm' $where ) AS c2)";
                }else{
